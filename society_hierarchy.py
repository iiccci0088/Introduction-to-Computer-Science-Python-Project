...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        if not self._subordinates:
            ...
            return
        for c in range(len(self._subordinates)):
            if self._subordinates[c].cid > subordinate.cid:
                self._subordinates.insert(c, subordinate)
                return
        self._subordinates.append(subordinate)
    ...
    def remove_subordinate(self, cid: int) -> None:
        ...
        """
        for c in range(len(self._subordinates)):
            if self._subordinates[c].cid == cid:
                self._subordinates[c].set_superior(None)
                self._subordinates.pop(c)
                break
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        for subordinate in self._subordinates:
            found = subordinate.get_citizen(cid)
...
class Society:
    ...
    def get_all_citizens(self) -> list[Citizen]:
        ...
        True
        """
        all_subs = self.get_head().get_all_subordinates()
        for c in range(len(all_subs)):
            if all_subs[c].cid > self.get_head().cid:
                all_subs.insert(c, self.get_head())
                return all_subs
        all_subs.append(self.get_head())
        return all_subs
    ...
    def get_citizens_with_job(self, job: str) -> list[Citizen]:
        ...
        True
        """
        all_c = self.get_all_citizens()
        job_citizens = []
        for c in all_c:
            if c.job == job:
                job_citizens.append(c)
        return job_citizens
    ...
    def change_citizen_type(self, cid: int,
        ...
        sup = c.get_superior()
        subs = c.get_direct_subordinates()
        if isinstance(c, DistrictLeader):
            new_c = Citizen(c.cid, c.manufacturer, c.model_year,
                            c.job, c.rating)
        else:
            new_c = DistrictLeader(c.cid, c.manufacturer, c.model_year,
                                   c.job, c.rating, district_name)
        for sub in subs:
            c.remove_subordinate(sub.cid)
            new_c.add_subordinate(sub)
        if sup is None:
            self.set_head(new_c)
        else:
            sup.remove_subordinate(c.cid)
            sup.add_subordinate(new_c)
        return new_c
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        # Remove all superior's subs
        for s in sup_sub:
            sup.remove_subordinate(s.cid)
        # Remove all citizen's subs
        for s in sub:
        ...
        if sup.get_superior() is None:
            ...
            # Remove the superior from sup's superior's subordinates list
            sup_sup.remove_subordinate(sup.cid)
            # Add citizen to sup's superior's subordinates list
            sup_sup.add_subordinate(citizen)

        # Add all superior's subs to citizen's subordinates list
        for s in sup_sub:
            if s.cid != citizen.cid:
                citizen.add_subordinate(s)
        # Add superior to citizen's subordinates list
        citizen.add_subordinate(sup)
        # Add citizen's subs to superior's subordinates list
        for s in sub:
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        subs = c.get_direct_subordinates()
        if sup is None:
            highest_rated = c.get_highest_rated_subordinate()
            self.set_head(highest_rated)
            highest_rated.set_superior(None)
        else:
            for s in subs:
                c.remove_subordinate(s.cid)
                sup.add_subordinate(s)
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> list[Citizen]:
        ...
        True
        """
        all_subs = self.get_all_subordinates()
        for c in range(len(all_subs)):
            if all_subs[c].cid > self.cid:
                all_subs.insert(c, self)
                return all_subs
        all_subs.append(self)
        return all_subs
